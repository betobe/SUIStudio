const tryRequire = ({category, component, theme}) => {
  ...
  require.ensure([], () => {
    try {
      let bundler
      if (!theme) {
        bundler = reqComponentsSCSS(`./${category}/${component}/src/index.scss`)
      } else {
        bundler = reqThemePlayGround(`./${category}/${component}/themes/${theme}.scss`)
      }
      bundler(src => {
        if (!theme) {
