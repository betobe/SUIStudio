const tryRequire = ({category, component}) => {
  ...
  const reqComponentsSCSS = require.context(`bundle-loader?lazy!${__BASE_DIR__}/components`, true, /^\.\/\w+\/\w+\/src\/index\.scss/)
  const reqComponentsPlayGround = require.context(`bundle-loader?lazy!raw-loader!${__BASE_DIR__}/demo`, true, /^.*\/playground/)
  const reqContextPlayGround = require.context(`bundle-loader?lazy!${__BASE_DIR__}/demo`, true, /^.*\/context\.js/)
  const reqRouterPlayGround = require.context(`bundle-loader?lazy!${__BASE_DIR__}/demo`, true, /^.*\/routes\.js/)
